...
from .forecast_service import ensure_seed_forecasts, calculate_recommendations, provider_status
...
def home(request:Request, user=Depends(current_user)):
    ...
    return templates.TemplateResponse('login.html', {'request':request,'csrf':'anonymous','error':None})
...
def login(request:Request, username:str=Form(...), password:str=Form(...), db:OrmSession=Depends(get_db)):
    ...
    if rate_limited(key): return templates.TemplateResponse('login.html', {'request':request,'csrf':'anonymous','error':'Invalid username or password.'}, status_code=429)
    user=db.query(User).filter(User.username_lower==username.lower()).first()
    if not user or not verify_password(password,user.password_hash):
        record_failure(key); return templates.TemplateResponse('login.html', {'request':request,'csrf':'anonymous','error':'Invalid username or password.'}, status_code=401)
...
def surf(request:Request, user=Depends(require_user), db:OrmSession=Depends(get_db)):
    date=datetime.now(ZoneInfo(settings.timezone)).date(); recs=db.query(DailyRecommendation).filter(DailyRecommendation.score_date==date).all()
    by={r.daypart:r.spot_score for r in recs}; spots=db.query(SurfSpot).order_by(SurfSpot.name).all(); latest=db.query(MarineForecast).order_by(desc(MarineForecast.fetched_at)).first()
    spot_summaries={s.id:db.query(SpotScore).filter(SpotScore.spot_id==s.id,SpotScore.score_date==date).order_by(desc(SpotScore.score)).first() for s in spots}
    return templates.TemplateResponse('surf.html', {'request':request,'user':user,'date':date,'recommendations':by,'spots':spots,'spot_summaries':spot_summaries,'csrf':request.state.csrf,'provider_status':provider_status(db),'newest_data':latest.fetched_at if latest else None})
...
def spot_detail(slug:str, request:Request, user=Depends(require_user), db:OrmSession=Depends(get_db)):
    spot=db.query(SurfSpot).filter(SurfSpot.slug==slug).first()
    if not spot: raise HTTPException(404)
    date=datetime.now(ZoneInfo(settings.timezone)).date(); scores=db.query(SpotScore).filter(SpotScore.spot_id==spot.id,SpotScore.score_date==date).all(); by={s.daypart:s for s in scores}
    latest=db.query(MarineForecast).filter(MarineForecast.spot_id==spot.id).order_by(desc(MarineForecast.fetched_at)).first()
    return templates.TemplateResponse('spot_detail.html', {'request':request,'user':user,'spot':spot,'scores':by,'csrf':request.state.csrf,'provider_status':provider_status(db),'latest':latest})
@app.get('/health')
