...
python -m app.tools.spot_intelligence status --limit 25
python -m app.tools.spot_intelligence inspect-run 7 --limit 50
python -m app.tools.spot_intelligence explain-point 99
```

IDs must be positive. Status and inspect limits are capped at 100. JSON is deterministic, bounded, and secret-redacted. A dry run always calculates afresh and writes nothing. An equivalent completed run is reused unless forced. A non-dry failure after input validation leaves one failed, redacted audit run and no partial points. Retry normally after correcting the cause; use `--force` only to preserve a distinct rerun of identical inputs.
...
The engine emits structured hazards, uncertainty factors, and detailed provenance including upstream point/run/version/configuration, exact rules snapshot/hash, formula version, scope/hash, fallbacks, and warnings. Consensus agreement, freshness, completeness, spatial relevance, and confidence-input values are uncertainty inputs only; this is **not final confidence**.
